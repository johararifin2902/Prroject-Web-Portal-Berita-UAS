...
<?php 
    ...
    $get_existing_contact = "SELECT * FROM contact_us WHERE admin_email = '$_SESSION[email]' ORDER BY id DESC LIMIT 1";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // if everything got from user
        if (!empty($email) && !empty($address) && !empty($phn_no) && !empty($fax)) {
            // database connection
            require_once "include/connection.php";

            $admin_email = $_SESSION['email'];
            $add_details = "INSERT INTO contact_us (address, phn, email, fax, admin_email) VALUES ('$address', '$phn_no', '$email', '$fax', '$admin_email')";
                        ...
                        <div class="card-body pt-5 shadow">                       
                            ...
                            <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" value="<?php echo empty($phn_no) ? $phn_no_invalid : $phn_no; ?>" name="phn">
                                    <?php echo $phn_no_err; ?> 
                                </div>
                                <div class="form-group">
                                    <label>Email :</label>
                                    <input type="text" class="form-control" value="<?php echo empty($email) ? $email_invalid : $email; ?>" name="email">
                                    <?php echo $email_err; ?>
                                </div>
                                <div class="form-group">
                                    <label>Fax :</label>
                                    <input type="text" class="form-control" value="<?php echo empty($fax) ? $fax_invalid : $fax; ?>" name="fax">
                                    <?php echo $fax_err; ?>
                                </div>
